...
const CONFIG = {
  ...
  GITHUB_REPO:   'YOUR_REPO_NAME',           // ← REPLACE with your repo name (e.g. 'personal-finance-assistant')
  GITHUB_BRANCH: 'main',
 ...
 * Steps: In Apps Script editor → select 'storeToken' from dropdown → Run
 * After running, delete the token string from this code and save.
 */
function storeToken() {
 ...
 * GitHub Actions then rebuilds app.js + ledgr-mobile.html and deploys.
 */
function onSheetChange() {
  try {
    ...
    // Uncomment to receive email alerts on failure:
...
function formatMonth(d, style) {
  ...
  if (style === 'short') {
    return date.toLocaleDateString('en-US', { month: 'short', year: 'numeric' }); // "Jan 2026"
  }
  return date.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });    // "January 2026"
}
